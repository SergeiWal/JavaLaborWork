package fit.bstu.walko.Lab13.commands.constants;

public class AuthorizationConst {
    //Login jsp
    public final static String LOGIN = "login";
    public final static String PASSWORD = "password";
    //ERROR MESSAGE
    public final static String ERROR_MESSAGE = "";
    public final static String ERROR_MESSAGE_TEXT = "Неверный логин или пароль, заполните все поля";
    public final static String AUTHENTICATION_ERROR_TEXT = "Неверный логин или пароль";
    public final static String REGISTER_ERROR_MESSAGE_IF_EXIST = "Выберите другое имя, такой пользователь сущетвует";
    public final static String REGISTER_ERROR = "errorRegister";
    public final static String COMMAND_WELCOME = "/controller?command=welcome";
    //REGISTER JSP
    public final static String NAME_FOR_REGISTER = "newLoginName";
    public final static String PASSWORD_FOR_REGISTER = "newPassword";
}
